...
|---|---|
| 계산 로직 명세 + 공유 테스트 벡터 | ✅ |
| Kotlin 코어 로직 + 테스트 (부하·ACWR·추천·부위피로·직렬화) | ✅ 이 저장소 CI 환경에서 실행 검증 (9 테스트 통과) |
| Swift 코어 로직 + 테스트 | ✅ 작성 완료 — macOS에서 `swift test` 실행 필요 |
| 기록 CRUD (추가·목록·수정·삭제 + 로컬 저장) | ✅ 양 플랫폼 작성 완료 — 각 IDE에서 빌드 확인 필요 |
| 홈 대시보드 · 주간 분석 · 추천 상세 | ✅ 양 플랫폼 작성 완료 — 각 IDE에서 빌드 확인 필요 |
| Supabase 연동 · 소셜 로그인 (카카오·Google·네이버·Apple) | 📋 가이드 작성 (docs/AUTH.md) — 구현 예정 |
| 온보딩 프로필 · 로그인/가입 화면 | 📋 예정 (인증 연동과 함께) |

> ⚠️ 앱 UI 계층(Android `app`, iOS `Loadcast`)은 리눅스 CI 환경에 Android SDK / Swift 툴체인이
> 없어 **컴파일 검증은 각 플랫폼 IDE에서 첫 빌드 시 필요**합니다. 계산 코어만 실행 검증되었습니다.
